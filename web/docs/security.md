...
5. Klunok [drops privileges](./cli.md#-d-path-to-a-file-or-directory-whose-owners-identity-will-be-used-when-running-klunok), and exits if it cannot.
...
Therefore, Klunok monitors only the current working directory for file writes by default.
This avoids receiving read-only file descriptors of, for example, `/etc/shadow`.
Note that without [the `-e` command-line option](./cli.md#-e-path-to-a-directory-that-contains-executable-files) Klunok monitors `/` for file executions to recognize editors.
